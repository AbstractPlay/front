...
function Tournaments() {
  ...
  const data = useMemo(
    () =>
      tourneys
        .map(({tid, metaGame, variants, archived, dateEnded, place, participants, score}) => {
          ...
          return {
            ...
            gameName,
            variants,
            tourneyName: `${gameName} (${variants.length === 0 ? "no variants" : variants.join("|")})`,
            dateEnded,
            archived,
            place,
            participants,
        ...
        [
            columnHelper.accessor("tourneyName", {
              ...
              cell: (props) => (
                <>
                  <Link
                    to={`/tournament${props.row.original.archived ? `/${props.row.original.meta}` : ""}/${props.row.original.id}`}
                  >
                    {props.getValue()}
                  </Link>
                </>
