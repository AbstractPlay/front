        ...
        <SummaryContext.Provider value={[summary, summarySetter]}>
            <article id="leaderboard">
                ...
                <div className="content has-text-centered">
                    <p>Statistics are generated every Sunday at 6am UTC.<br/>
                    Records analyzed include games completed between {summary !== null ? formatDate(new Date(summary.oldestRec)) : "??"} and {summary !== null ? formatDate(new Date(summary.newestRec)) : "??"} ({recDays} day{recDays !== 1 ? "s" : ""}, or {recYears} year{recYears !== 1 ? "s" : ""}).<br/>
                    In that time, {summary !== null ? summary.numPlayers : "??"} players have played {summary !== null ? summary.numGames : "??"} games.</p>
                    <p className="help">Please note that the ratings given here are dynamically generated and may differ from the static rating visible elsewhere. We're working on harmonizing things eventually.</p>
                </div>
                <div className="field has-text-centered">
                    <label className="label">Select a statistic</label>
                    <div className="control">
                        <div className="select" onChange={(e) => statSelectedSetter(e.target.value)}>
                            <select>
                                <option value="" key=""></option>
                            {modules.map(([name, ]) => {
                                return <option value={name} key={name}>{t(`stats_module_${name}`)}</option>
                            })}
                            </select>
                        </div>
                    </div>
                ...
                <div className="field is-grouped topPad">
                    <div className="control">
                        <a href="http://records.abstractplay.com/_summary.json">
                            <button className="button is-small apButton">Download all summary stats</button>
                        </a>
                    </div>
                    <div className="control">
                        <a href="http://records.abstractplay.com/ALL.json">
                            <button className="button is-small apButton">Download all game reports</button>
                        </a>
                    </div>
                </div>
            </article>
