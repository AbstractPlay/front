export class GameNode {
  ...
  AddChild(move, state, toMove) {
    for (let i = 0; i < this.children.length; i++) {
      if (this.children[i].move === move) return i;
    }
  ...
  UpdateOutcome() {
    ...
    this.children.forEach((child) => {
      if (
        (this.toMove === 0 && child.outcome === 1) ||
        (this.toMove === 1 && child.outCome === -1)
      )
        lose = true;
      if (
        (this.toMove === 0 && child.outcome !== -1) ||
    ...
    this.outcome = 0;
    if (this.toMove === 0 && lose) this.outcome = 1;
    if (this.toMove === 0 && win) this.outcome = -1;
    if (this.toMove === 1 && lose) this.outcome = -1;
    if (this.toMove === 1 && win) this.outcome = 1;
