...
Edit English keys in `public/locales/en/apfront.json` (and bundled `src/locales/en/apfront.json` when needed for the default bundle). Many keys use dynamic paths (for example `categories.${tag}`); do not run automated extraction tools that prune unreferenced keys.
...
- **Weblate** edits `public/locales/` only; human edits are authoritative when English is unchanged
- **CI** (`bin/translate.mjs` on **develop** deploy only) machine-translates new/changed English and maintains `locale-src/` stamps; **main** publishes locales from the merged develop tree without calling Gemini
