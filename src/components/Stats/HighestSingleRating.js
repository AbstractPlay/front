...
function HighestSingleRating(props) {
    ...
    const data = useMemo( () => summary.ratings.highest.map(({user: userid, game, rating}) => {
        let name = "UNKNOWN"
    ...
    }).sort((a, b) => b.rating - a.rating), [summary, userNames]);
    ...
    return (
        <TableSkeleton data={data} columns={columns} sort={[{id: "rating", desc: true}]} />
    );
}
