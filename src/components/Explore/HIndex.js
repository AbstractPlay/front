...
function HIndex({ toggleStar, openImgModal }) {
  ...
  const [sorting, setSorting] = useState([{ id: "hindex", desc: true }]);
  ...
  return (
    <>
      <div className="container" style={{ paddingBottom: "1em" }}>
        <h1 className="subtitle">Highest h-index</h1>
      </div>
      <ReactMarkdown rehypePlugins={[rehypeRaw]} className="content">
        A game's `h-index` is the number of different players who have played
        that game at least that many times. So an `h-index` of 5 means that 5
        different players have played that game at least 5 times.
      </ReactMarkdown>
