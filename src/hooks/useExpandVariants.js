...
export function useExpandVariants(metaGame) {
  ...
  useEffect(() => {
    ...
    if (all !== undefined) {
      setVariantMap(new Map(all.map((rec) => [rec.uid, rec.name])));
      setVariantGroups(new Set(all.map((rec) => rec.group).filter(Boolean)));
      setVarId2Group(new Map(all.map((rec) => [rec.uid, rec.group])));
    } else {
