...
function AvgRatings(props) {
    ...
    useEffect(() => {
        ...
        for (const obj of summary.ratings.avg) {
            const weighted = summary.ratings.weighted.find(u => u.user === obj.user);
            lst.push({user: obj.user, avg: obj.rating, weighted: weighted.rating})
            joinedSetter(lst);
        }
    }, [summary]);

    const data = useMemo( () => joined.map(({user: userid, avg, weighted}) => {
        let name = "UNKNOWN"
    ...
    }).sort((a, b) => b.avg - a.avg), [joined, userNames]);
    ...
    return (
        <TableSkeleton data={data} columns={columns} sort={[{id: "avg", desc: true}]} />
    );
}
