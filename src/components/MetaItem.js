          ...
          { counts === undefined ? '' :
            <Fragment>
              {counts.currentgames === 0 ? <span>0 current games</span> :
                <span>
                  {`${counts.currentgames} `}
                  <Link to="/listgames" state={{"metaGame": game.uid, "type": "current" }}>{t("CurrentGamesCount", {count: counts.currentgames})} </Link> 
                </span>
              }
              <span>, </span>
              {counts.completedgames === 0 ? <span>0 completed games</span> :
                <span>
                  ...
                  <Link to="/listgames" state={{"metaGame": game.uid, "type": "completed" }}>{t("CompletedGamesCount", {count: counts.completedgames})}</Link>
                </span>
              }
              <span>, </span>
              {counts.standingchallenges === 0 ? <span>0 standing challenges</span> : 
                <span>
                  ...
                  <Link to="/challenges" state={{"metaGame": game.uid }}>{t("StandingChallengesCount", {count: counts.standingchallenges})}</Link>
                </span>
              }
              <span>, </span>
              {!counts.ratings || counts.ratings.length === 0 ? <span>0 rated players</span> :
                <span>
                  {`${counts.ratings} `}
                  <Link to="/ratings" state={{"metaGame": game.uid }}>{t("RatedPlayersCount", {count: counts.ratings})}</Link>
                </span>
              }
            </Fragment>
          }
