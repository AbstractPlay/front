  ...
  if (respond) {
    return (
      <li>
        ...
        <button
          ...
          onClick={() => handleChallengeResponseClick(challenge)}
        >
          {t("Respond")}
        </button>
      </li>
    ...
    if (challenge.numPlayers === 2) {
      if (otherplayers.length === 0) desc = game.name;
        ...
        desc = t("ChallengedTwoPlayers", {
          game: game.name,
          other: otherplayers[0],
        });
    } else {
      if (otherplayers.length === 0)
        desc = t("ChallengedNoOthers", { game: game.name });
      else
        desc = t("ChallengedOthers", {
          game: game.name,
    ...
    return (
      <li>
        ...
        <button
          className="button is-small apButton inlineButton"
          onClick={() => handleChallengeViewClick(challenge)}
        >
          {t("View")}
        </button>
      </li>
