...
- **bug** — status flow: open → triaged → monitoring → resolved | closed; attachments optional on create (images, `.txt`, `.json`); up to 3 files per follow-up comment; bug context captured from error pages
- **feature** — markdown body; attachments on create and comments (up to 3 files per comment: images, `.txt`, `.json`)
