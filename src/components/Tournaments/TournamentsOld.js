...
function TournamentsOld(props) {
        ...
        <div className="column is-half is-offset-one-quarter">
          <div className="level smallerText has-text-centered">
            <div className="level-item">
              ...
              <button
                className="button is-small"
                onClick={() => oldTournamentsTable.setPageIndex(oldTournamentsTable.getPageCount() - 1)}
                disabled={!oldTournamentsTable.getCanNextPage()}
              >
                <span className="icon is-small">
            ...
            <div className="level-item">
              <p>
                Page{" "}
                <strong>{oldTournamentsTable.getState().pagination.pageIndex + 1}</strong> of{" "}
                <strong>{oldTournamentsTable.getPageCount()}</strong> (
                {oldTournamentsTable.getPrePaginationRowModel().rows.length} total tournaments)
              </p>
            </div>
            <div className="level-item">
  ...
  return (
    <>
      <Helmet>
          <meta property="og:title" content={`${metaGameName}: Tournament History`} />
          <meta property="og:url" content={`https://play.abstractplay.com/tournamenthistory/${metaGame}`} />
          <meta property="og:description" content={`Historical ${metaGameName} tournaments`} />
      </Helmet>
            ...
            <div className="card-content">
              { oldTournamentsData.length === 0 ?
                ...
                <>
                  ...
                  >
                    <thead>
                      {oldTournamentsTable.getHeaderGroups().map((headerGroup) => (
                  ...
                  { oldTournamentsData.length > 10 ? oldTournamentsTableNavigation : null }
                </>
              }
            </div>
          </div>
